...
export default function DepthCarousel({
  ...
  const autoTimerRef = useRef<NodeJS.Timeout | number | null>(null);
  ...
  useEffect(() => {
    ...
    let focused = false;
    const stop = () => {
      if (autoTimerRef.current) clearInterval(autoTimerRef.current as number);
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            />
            {item.title && (
              <div className="absolute inset-0 p-6 flex flex-col justify-between text-white z-10 bg-gradient-to-t from-black/90 via-black/30 to-black/20 pointer-events-none">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-semibold tracking-widest uppercase bg-white/20 backdrop-blur-md border border-white/30 px-3 py-1 rounded-full">
                    Featured Residence
                  </span>
                  <span className="font-playfair italic text-xl font-bold">Aura</span>
                </div>
                <div className="flex flex-col gap-1.5 text-left">
                  <h3 className="font-playfair italic text-2xl font-bold leading-snug">{item.title}</h3>
                  <div className="flex items-center justify-between text-sm text-white/90">
                    <span className="text-[#e8702a] font-bold text-lg">{item.price}</span>
